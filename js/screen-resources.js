...
//                    current goals. Not switched on yet: it needs Spotify
//                    credentials on THIS network, and the old site's version
//                    can't be borrowed (it doesn't know these agent numbers).
...
import { el, esc, showOverlay, toast } from './state.js'
...
const TOOLS = [
  ...
  },
  {
    ...
    tag: 'Not wired up yet',
    body: 'Builds a real Spotify playlist around the current goals. Needs Spotify credentials on this network before it can run — the old site\'s version can\'t be reused, it doesn\'t know these agent numbers.',
    action: () => toast('Playlist generation is not wired up on this network yet.'),
  },
  {
    icon: '📻',
    name: 'BOTZ',
...
export function renderResources(container, state) {
  ...
  for (const t of TOOLS) {
    ...
    `
    if (t.action) card.onclick = () => t.action(state)
    list.appendChild(card)
  }
  wrap.appendChild(list)

  const foot = el('div', 'pack-foot')
  foot.innerHTML = 'BOTZ runs inside the game now. Candy Star needs Spotify '
    + 'credentials on this network before it can be switched on.'
